...
import (
	...
	"github.com/apecloud/myduckserver/pgtest"
...
type ScriptTestAssertion = pgtest.ScriptTestAssertion
...
func RunReplicationScripts(t *testing.T, scripts []ReplicationTest) {
	...
	containerName, dsn, _, err := StartPostgresServer()
	require.NoError(t, err)
	defer func() {
		err := exec.Command("docker", "kill", containerName).Run()
		require.NoError(t, err)
	}()
...
func RunReplicationScript(t *testing.T, dsn string, script ReplicationTest) {
	...
	// primaryDns is the connection to the actual postgres database.
	// If you have postgres running on a different port, you'll need to change this.
	primaryDns := dsn + "?sslmode=disable"

	ctx, pgServer, replicaConn, close, err := pgtest.CreateTestServer(t, findFreePort())
	require.NoError(t, err)
	defer func() {
	...
	t.Run(script.Name, func(t *testing.T) {
		runReplicationScript(ctx, t, script, pgServer, replicaConn, primaryDns)
	})
}
...
func runReplicationScript(
	...
	for _, query := range script.SetUpScript {
		...
		if handlePseudoQuery(t, server, query, r) {
			continue
		}
	...
	for _, assertion := range script.Assertions {
		t.Run(assertion.Query, func(t *testing.T) {
			...
			// handle logic for special pseudo-queries
			if handlePseudoQuery(t, server, assertion.Query, r) {
				return
			}
			...
			for retries := 0; retries < numRetries; retries++ {
				...
				if assertion.SkipResultsCheck || assertion.ExpectedErr != "" {
					...
					require.NoError(t, err)
					readRows, err := pgtest.ReadRows(rows, true)
					require.NoError(t, err)
					normalizedRows := pgtest.NormalizeExpectedRow(rows.FieldDescriptions(), assertion.Expected)
...
func handlePseudoQuery(t *testing.T, server *pgserver.Server, query string, r *logrepl.LogicalReplicator) bool {
	...
	case startReplication:
		go func() {
			require.NoError(t, r.StartReplication(server.NewInternalCtx(), slotName))
		}()
