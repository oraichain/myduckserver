...
public class MySQLTest
{
    public class Tests
    {
        ...
        {
            string connectionString = $"Server={ip};Port={port};User Id={user};Password={password};";
            try
            {
        ...
        public class Test
        {
            ...
            public bool Run(MySqlCommand cmd)
            {
                try
                {
    ...
    public static void Main(string[] args)
    {
        if (args.Length < 5)
        {
            Console.Error.WriteLine("Usage: MySQLTest <ip> <port> <user> <password> <testFile>");
            Environment.Exit(1);
        }
