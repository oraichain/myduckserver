...
- **Accelerate MySQL & Postgres analytics** by running analytical queries on your MySQL & Postgres data at speeds several orders of magnitude faster 🚀
...
- **Zero-ETL**: Just start replication and go! MyDuck can act as a MySQL replica or Postgres standby that replicates data from your primary server in real-time, so you can start querying immediately. There’s no need to set up complex ETL pipelines.
...
- **Standalone Mode**: MyDuck can run in standalone mode without replication. In this mode, it is a drop-in replacement for MySQL, but with a DuckDB heart. You can `CREATE TABLE`, transactionally `INSERT`, `UPDATE`, and `DELETE` data, and run blazingly fast `SELECT` queries.
...
For full analytical power, connect using the Postgres port and write DuckDB SQL directly:
